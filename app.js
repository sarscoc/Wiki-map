...
async function fetchPageLinks(title){
  ...
  for(let page=0;page<3;page++){
    ...
    const obj=Object.values(data.query?.pages||{})[0];
    for(const link of obj?.links||[])links.add(link.title);
    cont=data.continue?.plcontinue;if(!cont)break;
  }
  return links;
}
...
async function enrichWikipediaLinks(items){
  ...
  for(let i=0;i<withWiki.length;i+=5){
    const batch=withWiki.slice(i,i+5);
    setStatus(`Wikipediaリンクを取得中 ${Math.min(i+batch.length,withWiki.length)}/${withWiki.length}`);
    await Promise.all(batch.map(async p=>{try{p.links=await fetchPageLinks(p.wikiTitle)}catch(e){console.warn('links',p.wikiTitle,e)}}));
    if(i+5<withWiki.length)await sleep(80);
  }
}
...
async function loadArea(){
  els.load.disabled=true;els.load.textContent='読み込み中…';selectedTopic=null;els.selection.hidden=true;els.sheet.classList.remove('open');
  try{
    setStatus('Wikidataから地点を取得中…');
    places=await fetchWikidataPlaces(map.getBounds());
    renderMarkers();
    const wikiCount=places.filter(p=>p.wikiTitle).length;
    setStatus(`${places.length}地点 / Wikipedia ${wikiCount}記事`);
    await enrichWikipediaLinks(places);
    topics=buildTopics(places);renderTopics();renderMarkers();
    setStatus(`${places.length}地点・${topics.length}トピック`);
    els.sheet.classList.remove('has-results');void els.sheet.offsetWidth;els.sheet.classList.add('has-results');
  }catch(e){console.error(e);setStatus('読み込みに失敗しました。範囲を少し狭めて再試行してください。')}
  finally{els.load.disabled=false;els.load.textContent='この範囲を読み込む'}
}

els.load.addEventListener('click',loadArea);
els.handle.addEventListener('click',()=>{const open=els.sheet.classList.toggle('open');els.handle.setAttribute('aria-expanded',String(open))});
els.clear.addEventListener('click',clearTopic);
map.on('movestart',()=>{if(!els.load.disabled)setStatus('移動後「この範囲を読み込む」で更新')});
